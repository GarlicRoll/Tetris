...
public class Figure {
	...
	private boolean appearance_check(boolean[][] placement_table_to_check, boolean[][] matrix_placement_table, int matrix_height, int matrix_width) {
		// если блок удраяется о блок при появлении
	...
	public boolean set_type(boolean[][] matrix_placement_table, int matrix_height, int matrix_width) { // случайный вид фигуры
		Random random = new Random();
		int type_number = random.nextInt(7);
		
		// семь классических фигур
		if (type_number == 0) {
			this.placement_table = new boolean[][] { { true, true }, { true, true } };
		} else if (type_number == 1) {
			this.placement_table = new boolean[][] { { false, true, false }, { true, true, true } };
		} else if (type_number == 2) {
			this.placement_table = new boolean[][] { { true, true, true, true } };
		} else if (type_number == 3) {
			this.placement_table = new boolean[][] { { false, true, true }, { true, true, false } };
		} else if (type_number == 4) {
			this.placement_table = new boolean[][] { { true, true, false }, { false, true, true } };
		} else if (type_number == 5) {
			this.placement_table = new boolean[][] { { false, false, true }, { true, true, true } };
		} else if (type_number == 6) {
			this.placement_table = new boolean[][] { { true, false, false }, { true, true, true } };
		}
		this.height = placement_table.length;
		this.width = placement_table[0].length;
		if (appearance_check(placement_table, matrix_placement_table, matrix_height, matrix_width) == true) { // проверка на возможность появления
			add_blocks();
			return true;
		}
		else {
