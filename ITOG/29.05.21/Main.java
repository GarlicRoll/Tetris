...
public class Main {
	...
	public static void main(String[] args) {
		...
		ArrayList<String> backgrounds_paths = new ArrayList<>(); // фон
		backgrounds_paths.add(source + "background1.png");
		backgrounds_paths.add(source + "background2.png");
		backgrounds_paths.add(source + "background3.png");
		Random rand = new Random();
		Image background_image = new Image(display, backgrounds_paths.get(rand.nextInt(backgrounds_paths.size())));
		Label background_label = new Label(shell, SWT.BORDER | SWT.CENTER);
		background_label.setImage(background_image);
		background_label.setBounds(0, 0, game_table_width * BLOCK_SIZE, game_table_height * BLOCK_SIZE);
				...
				if (game_is_on == true) {
					if (matrix.figures.size() == 0) {
						...
						int x = random.nextInt(matrix.width - 3); // -3 для правой границы, чтобы никто там точно не
																	// появился
						if (matrix.add_figure(new Figure(x, 0, images, shell), matrix.placement_table, matrix.height,
								matrix.width) == false) { // добавление фигуры
							game_is_on = false;
						}
					...
					label_timer.setText(String.valueOf(total_minutes) + " : " + String.valueOf(total_seconds)); // установка
																												// текста
		...
		shell.addKeyListener(new KeyAdapter() { // управление
			public void keyPressed(KeyEvent e) {
				...
				// acceleration == true - фигура быстро падает вниз, поворачивать и двигать её
				// уже нельзя
